...
function PC({ plan, index, analysisId, userKeys, characters, creditBalance, onCreditsUsed }: { plan: any; index: number; analysisId?: string | null; userKeys: Set<string>; characters?: any[]; creditBalance?: number; onCreditsUsed?: (n: number) => void }) {
  ...
  const hasCredits = (creditBalance || 0) > 0
  const genWithCredits = async () => { setStatus('processing'); try{const r=await fetch('/api/generate-credits',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider:studio.provider,prompt:editedPrompt||prompt,negativePrompt:plan?.negativePrompt,duration:plan?.estimatedDuration||5,aspectRatio:'16:9',projectId:projectId||analysisId,shotId:`plan-${index}`})});const d=await r.json();if(!r.ok){setStatus('failed');return};onCreditsUsed?.(d.creditsUsed||1);setStatus('completed')}catch{setStatus('failed')} }
  const osFallbackPC = plan?.openSourceFallback
  const genOpenSourcePC = async () => { if(!osFallbackPC)return;setStatus('processing'); try{const r=await fetch('/api/generate-opensource',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({modelId:osFallbackPC.modelId,prompt:editedPrompt||prompt,negativePrompt:plan?.negativePrompt,projectId:projectId||analysisId,shotId:`plan-${index}`})});const d=await r.json();if(!r.ok){setStatus('failed');return};setStatus('completed')}catch{setStatus('failed')} }
