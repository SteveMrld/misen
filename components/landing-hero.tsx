...
export function LandingHero({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  return (
    <div className="landing-light">
      ...
      <section style={{ position: 'relative', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', maxWidth: 800, margin: '0 auto', padding: '0 24px', transform: `translateY(${scrollY * 0.12}px)` }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 99, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.15)', marginBottom: 32, fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>
            ...
            {heroScenes[currentHero]?.label || 'Cinéma'} — {fr ? 'Dirigé par MISEN' : 'Directed by MISEN'}
          </div>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 'clamp(2.8rem, 6.5vw, 5.5rem)', lineHeight: 1.05, letterSpacing: '-0.03em', color: '#fff', marginBottom: 24, fontWeight: 700, textShadow: '0 2px 20px rgba(0,0,0,0.3)' }}>
            ...
            <span style={{ background: 'linear-gradient(135deg, #C56A2D, #D4974A, #C56A2D)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              {fr ? 'MISEN réalise.' : 'MISEN directs.'}
            </span>
          </h1>
          ...
          <p style={{ fontSize: 'clamp(1rem, 2vw, 1.25rem)', color: 'rgba(255,255,255,0.8)', maxWidth: 560, margin: '0 auto 40px', lineHeight: 1.7, fontWeight: 400 }}>
            {fr
              ? "Le premier studio de production cinématographique piloté par IA. Un scénario entre, un film sort."
              : 'The first AI-powered cinematic production studio. A screenplay goes in, a film comes out.'
            }
          </p>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: 48, marginTop: 56 }}>
            {[
              { n: '30', l: fr ? 'Moteurs' : 'Engines' },
              { n: '11', l: fr ? 'Modèles IA' : 'AI Models' },
        ...
        <div className="landing-container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
            {[
              ...
              { step: '02', img: '/images/icon_step_analysez.png', title: fr ? 'Analysez' : 'Analyze', desc: fr ? '30 moteurs dissèquent chaque scène.' : '30 engines dissect every scene.' },
          ...
          <div style={{ maxWidth: 800, margin: '0 auto', position: 'relative' }}>
            ...
            {[
              ...
              },
              {
                ...
                desc: fr ? '30 moteurs IA dissèquent chaque mot : découpage en scènes, plans, émotions, personnages, mouvements de caméra, courbe de tension. Tout est automatique.' : '30 AI engines dissect every word: scene breakdown, shots, emotions, characters, camera movements, tension curve. Fully automatic.',
                where: 'MISEN',
                color: '#C56A2D',
              },
              {
      ...
      {/* ═══ BEFORE/AFTER — Montrer la magie ═══ */}
      <section className="landing-section" style={{ background: '#0F1115', overflow: 'hidden' }}>
        <div className="landing-container">
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            <p className="landing-label" style={{ color: '#C56A2D' }}>{fr ? 'LA MAGIE' : 'THE MAGIC'}</p>
            <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 'clamp(1.8rem, 3.5vw, 2.8rem)', color: 'white', letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: 16 }}>
              {fr ? 'Votre scénario entre.' : 'Your screenplay goes in.'}
              <br />
              <span style={{ color: '#C56A2D' }}>{fr ? 'Votre film sort.' : 'Your film comes out.'}</span>
            </h2>
          </div>

          {/* 3 columns: Before → MISEN → After */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 0, maxWidth: 900, margin: '0 auto' }}>
            {/* BEFORE */}
            <div style={{ padding: 32, background: 'rgba(255,255,255,0.03)', borderRadius: '16px 0 0 16px', border: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase' as const }}>{fr ? 'AVANT' : 'BEFORE'}</span>
              <div style={{ marginTop: 16, fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 1.8 }}>
                <p style={{ color: 'rgba(197,106,45,0.6)', marginBottom: 8 }}>EXT. DÉSERT — AUBE</p>
                <p>Dunes à perte de vue.</p>
                <p>Lumière dorée rasante.</p>
                <p style={{ marginTop: 8 }}>Une femme apparaît.</p>
                <p>Robe fluide dorée.</p>
                <p style={{ marginTop: 8, color: 'rgba(108,77,255,0.5)' }}>VOIX OFF</p>
                <p style={{ fontStyle: 'italic' }}>Au-delà du visible.</p>
              </div>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.15)', marginTop: 16 }}>{fr ? 'Un texte brut. Rien d\'autre.' : 'Raw text. Nothing else.'}</p>
            </div>

            {/* MISEN */}
            <div style={{ padding: 32, background: 'linear-gradient(135deg, rgba(197,106,45,0.08), rgba(108,77,255,0.08))', border: '1px solid rgba(197,106,45,0.15)', position: 'relative' }}>
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', color: '#C56A2D', textTransform: 'uppercase' as const }}>MISEN</span>
              <div style={{ marginTop: 16, fontSize: 11, lineHeight: 2.2 }}>
                {[
                  { label: fr ? '3 scènes détectées' : '3 scenes detected', color: '#C56A2D' },
                  { label: fr ? '6 plans décomposés' : '6 shots decomposed', color: '#C56A2D' },
                  { label: fr ? 'Émotion : contemplation → tension' : 'Emotion: contemplation → tension', color: '#6C4DFF' },
                  { label: fr ? 'Kling 3.0 → plans larges' : 'Kling 3.0 → wide shots', color: '#3B82F6' },
                  { label: fr ? 'Veo 3.1 → portrait voix off' : 'Veo 3.1 → voiceover portrait', color: '#10B981' },
                  { label: fr ? 'Runway → insert produit' : 'Runway → product insert', color: '#8B5CF6' },
                  { label: fr ? 'Prompts optimisés par modèle' : 'Prompts optimized per model', color: '#C56A2D' },
                  { label: fr ? 'Continuité vérifiée ✓' : 'Continuity verified ✓', color: '#10B981' },
                ].map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 4, height: 4, borderRadius: 99, background: item.color, flexShrink: 0 }} />
                    <span style={{ color: 'rgba(255,255,255,0.6)' }}>{item.label}</span>
                  </div>
                ))}
              </div>
              <p style={{ fontSize: 10, color: 'rgba(197,106,45,0.5)', marginTop: 16 }}>{fr ? '30 moteurs · <8 secondes' : '30 engines · <8 seconds'}</p>
            </div>

            {/* AFTER */}
            <div style={{ padding: 32, background: 'rgba(255,255,255,0.03)', borderRadius: '0 16px 16px 0', border: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase' as const }}>{fr ? 'APRÈS' : 'AFTER'}</span>
              <div style={{ marginTop: 16, fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 2 }}>
                {[
                  { label: fr ? 'Plan 1 — Plan large, dolly, f/2.0' : 'Shot 1 — Wide, dolly, f/2.0', sub: 'Kling 3.0' },
                  { label: fr ? 'Plan 2 — Insert, macro, f/2.8' : 'Shot 2 — Insert, macro, f/2.8', sub: 'Runway' },
                  { label: fr ? 'Plan 3 — Gros plan, fixe, f/1.4' : 'Shot 3 — Close-up, static, f/1.4', sub: 'Veo 3.1' },
                ].map((p, i) => (
                  <div key={i} style={{ marginBottom: 8 }}>
                    <p style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>{p.label}</p>
                    <p style={{ fontSize: 9, color: 'rgba(108,77,255,0.6)' }}>{p.sub}</p>
                  </div>
                ))}
                <div style={{ marginTop: 12, padding: '8px 12px', background: 'rgba(16,185,129,0.08)', borderRadius: 8, border: '1px solid rgba(16,185,129,0.15)' }}>
                  <p style={{ fontSize: 10, color: '#10B981', fontWeight: 600 }}>{fr ? '+ Storyboard · Timeline · Sous-titres · Musique IA' : '+ Storyboard · Timeline · Subtitles · AI Music'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 48 }}>
            <Link href={isLoggedIn ? '/magic' : '/register'} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 28px', borderRadius: 99, background: '#C56A2D', color: 'white', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
              {fr ? 'Essayer maintenant' : 'Try it now'} <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </section>

      {/* ═══ MCAP — Le routing intelligent ═══ */}
      <section className="landing-section" style={{ background: '#F6F7F9' }}>
        <div className="landing-container">
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            <p className="landing-label" style={{ color: '#6C4DFF' }}>{fr ? 'TECHNOLOGIE PROPRIÉTAIRE' : 'PROPRIETARY TECHNOLOGY'}</p>
            <h2 className="landing-h2">{fr ? 'Le bon modèle, pour chaque plan.' : 'The right model, for every shot.'}</h2>
            <p className="landing-subtitle" style={{ maxWidth: 600, margin: '0 auto' }}>
              {fr
                ? 'MCAP (Multi-Criteria AI Performance) analyse chaque plan sur 16 critères et route automatiquement vers le modèle IA optimal. Aucune autre plateforme ne fait ça.'
                : 'MCAP (Multi-Criteria AI Performance) analyzes every shot across 16 criteria and automatically routes to the optimal AI model. No other platform does this.'}
            </p>
          </div>
          ...
          <div style={{ maxWidth: 800, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 32 }}>
              {[
                {
                  type: fr ? 'Plan large, mouvement' : 'Wide shot, movement',
                  model: 'Kling 3.0',
                  reason: fr ? 'Meilleur en physique réaliste et mouvement 4K' : 'Best at realistic physics and 4K movement',
                  score: 94,
                  color: '#3B82F6',
                },
                {
                  type: fr ? 'Portrait, dialogue' : 'Portrait, dialogue',
                  model: 'Veo 3.1',
                  reason: fr ? 'Meilleur en lip-sync et synchronisation émotionnelle' : 'Best at lip-sync and emotional synchronization',
                  score: 91,
                  color: '#10B981',
                },
                {
                  type: fr ? 'VFX, effets visuels' : 'VFX, visual effects',
                  model: 'Sora 2',
                  reason: fr ? 'Meilleur en effets visuels et narration visuelle' : 'Best at visual effects and visual storytelling',
                  score: 96,
                  color: '#EC4899',
                },
              ].map((ex, i) => (
                <div key={i} style={{ background: 'white', borderRadius: 16, padding: 24, border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.03)' }}>
                  <p style={{ fontSize: 11, color: '#6B7280', fontWeight: 500, marginBottom: 12 }}>{ex.type}</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <span style={{ width: 10, height: 10, borderRadius: 99, background: ex.color }} />
                    <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 18, fontWeight: 700, color: '#0F1115' }}>{ex.model}</span>
                  </div>
                  <p style={{ fontSize: 11, color: '#9CA3AF', lineHeight: 1.5, marginBottom: 12 }}>{ex.reason}</p>
                  {/* Score bar */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1, height: 4, background: '#F3F4F6', borderRadius: 99, overflow: 'hidden' }}>
                      <div style={{ width: `${ex.score}%`, height: '100%', background: ex.color, borderRadius: 99 }} />
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 700, color: ex.color }}>{ex.score}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: 'white', borderRadius: 16, padding: 24, border: '1px solid rgba(0,0,0,0.06)' }}>
              <p style={{ fontSize: 11, fontWeight: 600, color: '#0F1115', marginBottom: 12 }}>{fr ? '16 axes d\'évaluation par plan' : '16 evaluation axes per shot'}</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {[
                  fr ? 'Résolution' : 'Resolution',
                  fr ? 'Durée max' : 'Max duration',
                  'Mouvement', 'Physique',
                  fr ? 'Mains' : 'Hands',
                  'Lip-sync',
                  fr ? 'Contrôle caméra' : 'Camera control',
                  fr ? 'Éclairage' : 'Lighting',
                  'VFX',
                  fr ? 'Cohérence' : 'Consistency',
                  fr ? 'Texte' : 'Text rendering',
                  fr ? 'Style' : 'Style range',
                  'Audio',
                  fr ? 'Vitesse' : 'Speed',
                  fr ? 'Coût' : 'Cost',
                  fr ? 'Souveraineté' : 'Sovereignty',
                ].map(axis => (
                  <span key={axis} style={{ padding: '4px 10px', borderRadius: 99, background: '#F3F4F6', fontSize: 10, color: '#6B7280', fontWeight: 500 }}>
                    {axis}
                  </span>
                ))}
              </div>
              <p style={{ fontSize: 10, color: '#9CA3AF', marginTop: 12 }}>
                {fr
                  ? 'Chaque modèle est scoré sur ces 16 axes. MISEN choisit automatiquement le meilleur pour chaque plan de votre film.'
                  : 'Each model is scored across these 16 axes. MISEN automatically picks the best for every shot in your film.'}
              </p>
            </div>
          </div>
        </div>
      ...
      <section id="showcase" className="landing-section" style={{ background: '#FFFFFF' }}>
        <div className="landing-container">
          <div style={{ textAlign: 'center', marginBottom: 80 }}>
            ...
            <h2 className="landing-h2">{fr ? 'Dirigé par MISEN, généré par IA' : 'Directed by MISEN, generated by AI'}</h2>
            <p className="landing-subtitle">{fr ? 'MISEN orchestre les meilleurs modèles IA pour chaque plan.' : 'MISEN orchestrates the best AI models for every shot.'}</p>
          </div>
        ...
        <div className="landing-container">
          ...
          <div style={{ marginTop: 64, textAlign: 'center' }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(15,17,21,0.2)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 16 }}>
              {fr ? 'Propulsé par 30 moteurs d\'analyse' : 'Powered by 30 analysis engines'}
            </p>
      ...
      <section className="landing-section" style={{ background: '#F6F7F9' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20 }}>
            {[
              { name: 'Free', price: '0€', desc: fr ? '3 projets' : '3 projects', features: fr ? ['30 moteurs d\'analyse', 'Assistant IA (3/mois)', 'Mode Simple', 'Export JSON'] : ['30 analysis engines', 'AI Assistant (3/mo)', 'Simple Mode', 'JSON Export'], hl: false },
