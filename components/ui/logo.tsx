...
export function Logo({ size = 'md', showText = true, className = '' }: { size?: 'sm' | 'md' | 'lg'; showText?: boolean; className?: string }) {
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      {showText && (
        <span className={`font-display tracking-tight text-gradient-copper ${size === 'sm' ? 'text-lg' : size === 'md' ? 'text-xl' : 'text-3xl'}`} style={{ letterSpacing: '-0.02em' }}>
          MISEN
        </span>
      )}
    </div>
...
export function LogoFull({ className = '' }: { className?: string }) {
  return (
    <div className={className}>
      ...
      <span className="font-display text-3xl text-gradient-copper" style={{ letterSpacing: '-0.02em' }}>MISEN</span>
