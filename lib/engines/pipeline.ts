...
export function runPipeline(scriptText: string, options: PipelineOptions = {}): AnalysisResult {
  ...
  const engineInsights = [
    { engine: 'Script Parser', icon: '📝', status: 'done' as const, insight: `${scenes.length} scènes extraites, ${plans.length} plans décomposés`, detail: `Détection: ${scenes.filter(s => s.type === 'EXT').length} EXT, ${scenes.filter(s => s.type === 'INT').length} INT${hasFlashbacks ? ', flashbacks détectés' : ''}` },
    { engine: 'Intent Parser', icon: '🧠', status: 'done' as const, insight: `Émotion dominante: ${dominantEmotion}`, detail: `${Object.entries(emotionDistribution).map(([e, c]) => `${e}(${c})`).join(', ')}` },
    { engine: 'Story Tracker', icon: '📖', status: 'done' as const, insight: `Arc narratif: ${tension?.globalArc || 'standard'}`, detail: `Tension moyenne: ${tension?.avgTension || 0}/100, pic: ${Math.max(...(tension?.curve?.map((c: any) => c.tension) || [0]))}/100` },
    { engine: 'Character Bible', icon: '👥', status: personnages.length > 0 ? 'done' as const : 'skip' as const, insight: personnages.length > 0 ? `${personnages.length} personnage${personnages.length > 1 ? 's' : ''} profilé${personnages.length > 1 ? 's' : ''}` : 'Aucun personnage détecté', detail: personnages.length > 0 ? `Tokens de cohérence générés pour ${Object.keys(characterBible).length} personnage(s)` : '' },
    { engine: 'Style Guard', icon: '🎨', status: 'done' as const, insight: `Preset: ${options.stylePreset || 'cinématique'}`, detail: `Bible de style appliquée à ${plans.length} plans` },
    { engine: 'Compliance', icon: '🛡️', status: worstCompliance.level === 'OK' ? 'done' as const : 'warn' as const, insight: `Score: ${worstCompliance.score}/100 — ${worstCompliance.level}`, detail: worstCompliance.flags.length > 0 ? `${worstCompliance.flags.length} alerte(s)` : 'Aucun contenu problématique' },
    { engine: 'Cinematic Grammar', icon: '🎬', status: 'done' as const, insight: `${Object.keys(cadrageDistribution).length} types de plans, ${Object.keys(cameraDistribution).length} mouvements`, detail: `Cadrages: ${Object.entries(cadrageDistribution).map(([c, n]) => `${c}(${n})`).join(', ')}` },
    { engine: 'Contextual Prompt', icon: '✍️', status: 'done' as const, insight: `${plans.length} prompts contextuels générés`, detail: `Adaptation au contexte narratif, éclairage, atmosphère par plan` },
    { engine: 'Consistency Inject', icon: '🔗', status: personnages.length > 0 ? 'done' as const : 'skip' as const, insight: personnages.length > 0 ? `Cohérence injectée dans ${plans.filter(p => p.personnages.length > 0).length} plans` : 'Pas de personnages à tracer', detail: 'Tokens visuels + style appliqués pour cohérence inter-plans' },
    { engine: 'Rec Engine (MCAP)', icon: '🤖', status: 'done' as const, insight: `${uniqueModels} modèle${uniqueModels > 1 ? 's' : ''} sélectionné${uniqueModels > 1 ? 's' : ''} sur 7`, detail: `Distribution: ${Object.entries(modelDistribution).map(([m, n]) => `${AI_MODELS[m as AIModelId]?.name || m}(${n})`).join(', ')}` },
    { engine: 'Model Syntax', icon: '⚙️', status: 'done' as const, insight: `Prompts adaptés à la syntaxe de chaque modèle`, detail: `Préfixes, suffixes, tokens négatifs spécialisés par API` },
    { engine: 'Negative Prompt', icon: '🚫', status: 'done' as const, insight: `${plans.length} prompts négatifs générés`, detail: `Anti-artefacts, anti-doigts, stabilisation par plan` },
    { engine: 'Continuity Tracker', icon: '🔍', status: continuity.score >= 80 ? 'done' as const : 'warn' as const, insight: `Score de continuité: ${continuity.score}/100`, detail: continuity.alerts?.length > 0 ? `${continuity.alerts.length} alerte(s) de raccord` : 'Continuité validée entre tous les plans' },
  ];
